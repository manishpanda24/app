...
async function readMediaFileBuffer(
  mediaDoc: { filename?: string | null; url?: string | null },
  serverURL: string,
): Promise<Buffer> {
  ...
  const localPath = path.join(MEDIA_DIR, mediaDoc.filename)
  if (fs.existsSync(localPath)) {
    return fs.readFileSync(localPath)
  }

  const fileUrl =
    mediaDoc.url || `${serverURL}/api/media/file/${encodeURIComponent(mediaDoc.filename)}`

  const response = await fetch(fileUrl)
  if (!response.ok) {
    throw new Error(
      `Could not read document file (${response.status}). Upload the .docx to Media first, or check storage configuration.`,
    )
  }
...
export async function processDocxImport(
  ...
  const fallbackTitle = mediaDoc?.filename
    ? path.basename(mediaDoc.filename, path.extname(mediaDoc.filename))
    : 'imported-document'
  ...
  const docxBuffer = await readMediaFileBuffer(mediaDoc, serverURL)
  ...
  const htmlResult = await mammoth.convertToHtml(
    { buffer: docxBuffer },
    {
      convertImage: mammoth.images.imgElement(async (image) => {
        ...
        const src =
          created.url || `${serverURL}/api/media/file/${encodeURIComponent(fileName)}`
  ...
  const plainResult = await mammoth.extractRawText({ buffer: docxBuffer })
