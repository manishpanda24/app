  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-end">
            <div className="lg:col-span-8 fade-in">
              ...
              <div className="flex flex-wrap items-center gap-4 mt-10">
                <Link to="/founder-services" className="btn-yellow">Explore services <ArrowRight className="w-4 h-4"/></Link>
                <Link to="/contact" className="btn-outline">Book a consultation</Link>
              </div>
              <div className="flex flex-wrap items-center gap-7 mt-12 text-[12px] text-amg-teal/80">
                <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-amg-turquoise-2"/>Pre-Seed · Seed · Series A</div>
                <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-amg-turquoise-2"/>NDA-first engagement</div>
                <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-amg-turquoise-2"/>Flat-fee, no surprises</div>
              </div>
            </div>

            <div className="lg:col-span-4 fade-in">
              <div className="card-turquoise shadow-soft-lg p-7 relative">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] tracking-[0.2em] uppercase font-semibold text-amg-teal/80">Featured Program</span>
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amg-teal text-amg-yellow text-[10px] tracking-[0.16em] uppercase font-semibold">
                    <span className="w-1.5 h-1.5 rounded-full bg-amg-yellow"/>Active
                  </span>
                </div>
                <div className="font-serif text-[26px] text-amg-teal leading-tight mt-3 font-medium">Investment Readiness Cohort</div>
                <div className="mt-1 text-[13px] text-amg-teal/75">6-module program · Spring 2026</div>
                <div className="my-5 h-px bg-amg-teal/20"/>
                <ul className="space-y-3 text-[13.5px] text-amg-teal">
        ...
        {/* Stats bar */}
        <div className="border-t border-amg-line bg-amg-cream-2/40">
          <div className="max-w-[1280px] mx-auto grid grid-cols-2 lg:grid-cols-4">
            {STATS.map((s, i) => {
              const colorClass = i === 0 ? 'text-amg-teal' : i === 1 ? 'text-amg-yellow-2' : i === 2 ? 'text-amg-turquoise-2' : 'text-amg-teal';
              return (
                <div key={i} className={`px-6 py-8 ${i!==0?'lg:border-l border-amg-line':''} ${i===1?'border-l border-amg-line':''} ${i===3?'lg:border-l border-amg-line':''} ${i>1?'border-t lg:border-t-0 border-amg-line':''}`}>
                  <div className={`font-serif text-[36px] lg:text-[44px] font-medium leading-none ${colorClass}`}>{s.value}</div>
                  <div className="mt-2 text-[11px] tracking-[0.18em] uppercase font-semibold text-amg-teal/80">{s.label}</div>
                </div>
              );
            })}
          </div>
      ...
      {/* PARTNERS MARQUEE */}
      <section className="py-12 border-y border-amg-line bg-amg-turquoise-bg/60 overflow-hidden">
        <div className="max-w-[1280px] mx-auto px-6 lg:px-10 flex items-center gap-8">
          <div className="text-[11px] tracking-[0.18em] uppercase font-semibold text-amg-teal whitespace-nowrap">Trusted by partners across</div>
          <div className="flex-1 overflow-hidden relative">
            <div className="marquee-track">
              {[...PARTNERS, ...PARTNERS].map((p, i) => (
                <div key={i} className="font-serif text-[20px] font-medium text-amg-teal/65 whitespace-nowrap">{p}</div>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="max-w-[1280px] mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 mb-14">
            <div className="lg:col-span-7">
              ...
              <h2 className="heading-display text-[40px] md:text-[56px] text-amg-teal">End-to-end fundraising <em>support</em>.</h2>
            </div>
      ...
      {/* METHOD */}
      <section className="py-24 px-6 lg:px-10 bg-amg-teal-bg/55 border-y border-amg-line">
        <div className="max-w-[1280px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
          <div className="lg:col-span-5 lg:sticky lg:top-28">
            ...
            <h2 className="heading-display text-[40px] md:text-[52px] text-amg-teal">A repeatable system <em>to close rounds</em>.</h2>
            <p className="mt-6 text-[15.5px] text-amg-teal/75 max-w-md leading-relaxed">
              Our four-phase method is the same we apply with accelerator cohorts and individual founders — calibrated, never templated.
            </p>
            <Link to="/founder-services" className="mt-7 inline-flex items-center gap-2 text-amg-turquoise-2 font-semibold link-under text-[13.5px]">See how we engage <ArrowRight className="w-4 h-4"/></Link>
          </div>
          <div className="lg:col-span-7 space-y-4">
            {[
              {n:'01', t:'Diagnose', d:'Audit deck, financial model, data room, narrative and legal structure.', color:'turquoise'},
              {n:'02', t:'Architect', d:'Sharpen the story, rebuild what is unfit, calibrate to your investor universe.', color:'yellow'},
              {n:'03', t:'Equip', d:'Polished investor-grade materials and defensible numbers, ready for diligence.', color:'turquoise'},
              {n:'04', t:'Engage', d:'Targeted, warm investor outreach with weekly pipeline reviews.', color:'yellow'},
            ].map((s, i) => (
              <div key={s.n} className="card-pro p-7 grid grid-cols-12 gap-5 items-start hover-lift relative overflow-hidden">
                <div className={`absolute left-0 top-0 bottom-0 w-1 ${s.color==='yellow'?'bg-amg-yellow':'bg-amg-turquoise'}`}/>
                <div className={`col-span-2 step-num ${s.color==='yellow'?'text-amg-yellow-2':'text-amg-turquoise-2'}`}>{s.n}</div>
                <div className="col-span-10">
                  <div className="font-serif text-[22px] text-amg-teal font-medium">{s.t}</div>
                  <div className="text-[14.5px] text-amg-teal/70 mt-1 leading-relaxed">{s.d}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 px-6 lg:px-10 bg-amg-yellow-bg/50">
        <div className="max-w-[1100px] mx-auto text-center">
          <Quote className="w-10 h-10 text-amg-yellow-2 mx-auto mb-6"/>
          <p className="font-serif text-[28px] md:text-[36px] leading-[1.25] text-amg-teal">
          ...
          <div className="mt-8 flex items-center gap-4 justify-center">
            <div className="w-11 h-11 rounded-full bg-amg-teal text-amg-yellow font-semibold flex items-center justify-center text-[14px]">SC</div>
            <div className="text-left">
              <div className="text-[14px] font-semibold text-amg-teal">Sarah Chen</div>
        ...
        <div className="max-w-[1280px] mx-auto">
          <div className="flex items-end justify-between gap-6 mb-12 flex-wrap">
            <div>
              ...
              <h2 className="heading-display text-[40px] md:text-[52px] text-amg-teal max-w-2xl">Outcomes, not opinions.</h2>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {TESTIMONIALS.map((t, i) => (
              <div key={i} className="card-pro p-8 shadow-soft relative overflow-hidden">
                <div className={`absolute top-0 left-0 right-0 h-1 ${i%3===0?'bg-amg-yellow':i%3===1?'bg-amg-turquoise':'bg-amg-teal'}`}/>
                <div className="flex items-center gap-1 mb-5">
                  {[...Array(5)].map((_,k)=>(<Star key={k} className="w-3.5 h-3.5 fill-amg-yellow text-amg-yellow"/>))}
                </div>
                <div className="font-serif text-[19px] leading-relaxed text-amg-teal">“{t.text}”</div>
                <div className="hr-thin my-6"/>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-amg-turquoise text-amg-teal font-semibold flex items-center justify-center text-[13px]">{t.initials}</div>
                  <div>
                    <div className="font-semibold text-[13.5px] text-amg-teal">{t.name}</div>
        ...
        <div className="max-w-[1280px] mx-auto">
          <div className="flex items-end justify-between gap-6 mb-12 flex-wrap">
            <div>
              ...
              <h2 className="heading-display text-[40px] md:text-[52px] text-amg-teal">Notes from the founder room.</h2>
            </div>
              ...
              return (
                <article key={a.slug} className="card-pro hover-lift overflow-hidden">
                  ...
                  <div className="p-7">
                    ...
                    <p className="mt-3 text-[14px] text-amg-teal/70 leading-relaxed line-clamp-3">{a.excerpt}</p>
                    <div className="hr-thin my-6"/>
                    <div className="flex items-center justify-between">
